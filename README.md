...
- Class routes now render a full quiz runtime for logged-in students when a quiz is mapped to the class: all questions are displayed on a single page in quiz order, each question still uses the same answer-choice UI, submission requires all questions to be answered, and final submission scoring/persistence continues through `teqcidb_quiz_attempts` + `teqcidb_quiz_answer_items` (with legacy `teqcidb_quiz_answers` compatibility writes) (pass thresholds: Initial = 75%, Refresher = 80%).
...
- Class-page quiz clients support authenticated quiz submission under `teqcidb/v1` via `POST /quiz/submit` (with legacy admin-ajax submit fallback). Requests require `X-WP-Nonce` (`wp_rest`), login, class/quiz access checks, and attempt ownership checks. Stable response fields include `ok`, `attempt_id`, `saved_at`, `server_version`, `message`, `score`, `passThreshold`, and `passed`; errors follow WordPress REST error schema (`code`, `message`, `data.status`).
