...
242. 2026-02-11: Added a logged-in-only `teqcidb_anet_get_token` AJAX endpoint that validates nonce/class ID, reads class pricing from `$wpdb->prefix . 'teqcidb_classes'`, generates server-side invoice/description/email values, and returns Accept Hosted tokens (or normalized SDK errors) via `TEQCIDB_AuthorizeNet_Service`.
243. 2026-02-11: Extended the student registration shortcode markup with per-class Accept Hosted checkout placeholders (token nonce, pay button data attributes, hidden token form, hidden iframe, and status area) rendered only for classes with numeric `classcost` values while preserving existing accordion behavior.
244. 2026-02-11: Added a shortcode-scoped Accept Hosted debug script that requests `teqcidb_anet_get_token`, logs verbose redacted checkout diagnostics, selects sandbox/live hosted URLs, and submits per-class token forms into hidden registration iframes without changing accordion behavior.
245. 2026-02-11: Updated registration checkout styling/behavior so Accept Hosted iframes render at 100% width and the expanded class panel recalculates max-height during iframe load/resize events to fully accommodate hosted payment content.
246. 2026-02-11: Set Accept Hosted registration iframes to a 800px minimum height to avoid inner iframe scrollbars and improve checkout form visibility while retaining full-width layout.
247. 2026-02-11: Extended Accept Hosted debug JS with a global `AuthorizeNetIFrame.onReceiveCommunication` receiver that parses communicator actions (`resizeWindow`, `cancel`, `transactResponse`), applies safe/redacted transaction logging, maps responses to the active class iframe UI, and adds a 60-second stuck-processing timeout notice.
248. 2026-02-11: Updated the public Authorize.Net iFrame communicator endpoint to return explicit CORS headers and handle OPTIONS preflight requests with a 204 response while preserving no-cache/noindex behavior.
249. 2026-02-11: Hardened the communicator route for cross-site iframe loading by removing frame-blocking headers, applying route-scoped frame-ancestors CSP for Authorize.Net hosts, preserving CORS/no-cache headers on GET+OPTIONS, and adding WP_DEBUG request-line logging without sensitive payload data.
