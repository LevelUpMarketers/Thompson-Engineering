...
273. 2026-02-16: Updated class-route attempt feedback lookup to require a quiz mapped to the current class (`teqcidb_quizzes.class_id`) and match attempts strictly by that quiz_id plus logged-in WP user_id, aligning status messaging with 0=passed, 1=failed, 2=in progress.
274. 2026-02-18: Added a new Quizzes admin submenu between Classes and Communications with Create a Quiz/Edit Quizzes tabs, including a schema-driven create form for teqcidb_quizzes (class_id, name, public token, status, settings JSON) and an edit-tab search/table scaffold for upcoming management workflows.
...
279. 2026-02-19: Added initial Quiz Questions management scaffolding inside each Edit Quizzes accordion panel, including per-quiz question-count detection from teqcidb_quiz_questions, no-questions empty-state messaging, and an Add Quiz Question button for upcoming authoring workflows.
280. 2026-02-19: Expanded Edit Quizzes quiz-question UI to render saved question entries with type-aware titles (Multiple Choice / True or False / Multi-Select), full-width prompt textareas sourced from teqcidb_quiz_questions.prompt, and prompt-save handling during quiz updates.
281. 2026-02-19: Added true_false-specific question editing UI in Edit Quizzes with a Select an Answer dropdown (True/False), loading/saving teqcidb_quiz_questions.choices_json in the format [{"correct":"true"}] or [{"correct":"false"}] while preserving prompt textarea updates.
282. 2026-02-19: Added dedicated Edit Quizzes question-card styling for .teqcidb-quiz-question and stronger visual hierarchy for .teqcidb-quiz-question__title so question titles remain larger/bolder than subordinate headings and labels.
283. 2026-02-19: Added Edit Quizzes-specific checkbox-grid styling (3-column layout, spacing, label min-height/baseline alignment) and scoped full-width class-association field card styling for the quiz class checkbox section.
284. 2026-02-19: Added per-question action rows for Edit Quizzes entries (Save Changes/Delete + inline spinner/feedback), with AJAX save handling for true_false questions that updates prompt and choices_json ({"correct":"true"|"false"}) in teqcidb_quiz_questions while leaving delete as a placeholder for a future update.
285. 2026-02-19: Added multi_select-specific quiz-question editing UI in Edit Quizzes (repeatable answer rows with per-option True/False selects + Add Another Answer), and extended per-question AJAX saves to persist multi-select choices_json entries (`id`, `label`, `correct` booleans) plus updated_at updates.
286. 2026-02-19: Refactored Edit Quizzes question cards into nested accordions so each question collapses by default with a summary showing question number/type and a 60-character prompt preview, reducing scrolling for large quizzes.
287. 2026-02-19: Tweaked Edit Quizzes question-accordion styling by adding 20px top spacing to Add Quiz Question and Add Another Answer buttons (scoped to the quiz edit list), and switched question summary previews to show full prompt text instead of truncated ellipses.
288. 2026-02-19: Added multiple_choice question editing behavior to mirror multi_select option rows, including JS auto-enforcement so only one option can be set to True at a time, and AJAX save validation requiring exactly one correct option before persisting choices_json.
289. 2026-02-19: Implemented per-question Delete functionality for Edit Quizzes question cards via AJAX (teqcidb_delete_quiz_question), with inline spinner/feedback, page reload on success, and sessionStorage-based restoration that re-opens the same quiz accordion after refresh.
290. 2026-02-19: Scoped quiz-edit accordion summary-cell styling to #teqcidb-quiz-list and applied explicit width (30%) + display (table-cell) alongside existing border/background/vertical-align/min-height so this layout change affects only Quizzes → Edit accordions.
291. 2026-02-19: Implemented Add Quiz Question creation flow with inline new-question UI (question text + type selector), type-specific builder panels, AJAX creation endpoint (teqcidb_create_quiz_question), and reload/restore behavior to return admins to the same quiz accordion after creating a question.
292. 2026-02-20: Enhanced Edit Quizzes bulk Save Changes handling to efficiently batch-load quiz questions, persist prompt/answer edits for existing true_false/multi_select/multiple_choice items, and auto-create valid unsaved new-question drafts from the same form submission (without requiring per-question Create Question clicks).
...
296. 2026-02-20: Added class-page quiz-access gating from teqcidb_classes so the quiz app only mounts when `allallowedquiz` is allowed, or when blocked classes explicitly include the logged-in wpuserid in `quizstudentsallowed`; otherwise a student-facing access-disabled message is shown instead of loading teqcidb-class-quiz-app.
297. 2026-02-20: Refined class-page quiz access UX by sourcing the welcome name from teqcidb_students first/last name, simplifying in-progress feedback to remove elapsed-save timing text, and using class-type-specific blocked messages ('Exam' for Initial and 'Refresher Quiz' for Refresher).
298. 2026-02-20: Updated class-page UX by auto-fading the restored-progress notice after 10 seconds, removing the welcome panel's placeholder description text, and rendering Class Resources from classresources JSON as labeled resource rows (Name + Type with URL link) with a new empty-state message when no resources exist.

299. 2026-02-25: Refactored quiz attempt persistence into distinct progress-save vs final-submit behavior so autosaves store only sanitized selections/current index while final submission reloads canonical stored progress, grades once, and then writes score/status/submitted timestamp.
300. 2026-02-25: Introduced row-per-question quiz answer storage (`teqcidb_quiz_answer_items`) with unique (attempt_id, question_id), added quiz-attempt `current_index` persistence, switched quiz runtime restore/autosave to use per-question rows with changed-row upserts, and updated final submission grading to aggregate canonical answer-item rows before writing final attempt results.
301. 2026-02-25: Added authenticated `teqcidb/v1` REST quiz endpoints (`/quiz/progress`, `/quiz/submit`) with wp_rest nonce + class/quiz/attempt ownership checks and autosave throttling, migrated class-page quiz client to prefer REST with `X-WP-Nonce` and feature-flagged AJAX fallback, and logged endpoint channel usage for rollout visibility.
302. 2026-02-25: Optimized class-page quiz autosave cadence by adding dirty/hash no-op suppression, interval-based saves (8s) plus boundary/submit/visibility-hidden immediates, and lightweight client save metrics hooks while preserving beforeunload beacon fallback.
303. 2026-02-25: Added indexed quiz/class join mapping (`teqcidb_quiz_classes`) with activation-time migration from legacy comma-separated quiz class IDs, switched class-page quiz lookup/access checks to join-based queries (with legacy fallback), and updated quiz admin create/edit saves to sync mapping rows on each write.
...
308. 2026-02-26: Wired Edit Students representative assignment saving into backend student saves by syncing `assigned_students[]` selections to `their_representative` on matched student records (including unassign on removal/toggle-off), and preloading representative assignment rows from existing relationships with hidden ID metadata for reliable round-trip edits.
309. 2026-02-26: Removed legacy front-end self-assignment plumbing by deleting the `teqcidb_assign_student_representative` AJAX endpoint and its unused representative-from-user helper in the AJAX class, while preserving representative visibility/editing for already-assigned students in the dashboard list.

310. 2026-02-26: Populated the student dashboard Payment History tab with class-linked payment cards (formatted date/time, total paid, transaction ID, invoice number), plus representative-ready 'Applied To' rendering that parses `multiplestudents` JSON and resolves student names/emails for multi-student payments.
311. 2026-02-26: Refined Payment History entries by renaming the card eyebrow to 'Class Name', replacing generic 'You' with the actual student full name for single-student payments, rendering multi-student applied-to lines in a future-ready per-line format with optional per-student amounts when present, and adding per-entry Print Invoice/Download Invoice buttons that reuse the registration receipt PDF generator.
312. 2026-02-26: Enhanced Student Dashboard Class History cards with class-page linking by making each class-name heading clickable and adding an 'Open Class Page' footer link per entry (new tab), backed by classurl joins from `teqcidb_classes` and normalized URL handling for relative/absolute class routes.
313. 2026-02-26: Tweaked Class History link UX by changing linked class-title color to the clickable accent orange (`#f8991d`) and updating the footer link copy to 'Click here to visit the Class Page.'
314. 2026-02-26: Began Register Students & Pay Online tab buildout for representatives by adding a 3-step UI (assigned-student multi-select checkboxes, single-class radio selection, and disabled-by-default Register & Pay button), wiring JS state management to enforce selection requirements, and including payment wrapper/form/iframe scaffolding plus selected-student payload plumbing for upcoming Accept Hosted multi-student checkout support.
315. 2026-02-26: Refined representative Register Students & Pay Online UI by scoping 20px top spacing to tab-specific class-history cards and registration action buttons, and updated Step 2 class loading to match student-registration visibility rules by excluding classes where `classhide = 1` (with upcoming-first ordering).
316. 2026-02-26: Added representative checkout pricing logic to `teqcidb_get_accept_hosted_token` so Accept Hosted amount now computes as class cost × selected students minus $50 per selected student with any association in `teqcidb_students.associations`, and wired token requests to send selected-student payloads for server-side calculation.
317. 2026-02-26: Simplified the student dashboard welcome banner copy to remove the trailing QCI-number sentence and keep only 'Welcome to your QCI Dashboard, [First Name]!'.
318. 2026-02-26: Updated representative dashboard checkout persistence so successful payments now save one aggregate paymenthistory row for the representative (with selected student WP IDs in `multiplestudents`), plus per-student paymenthistory rows using each student's discounted/non-discounted share, and per-student studenthistory rows with `registeredby` set to the representative's wpuserid (without creating a studenthistory row for the representative).
319. 2026-02-26: Fixed representative post-payment persistence branching to key off selected student IDs (not representative-flag truthiness), ensuring multi-student dashboard checkouts create per-student paymenthistory and studenthistory rows and do not fall back to creating only a representative studenthistory entry.
320. 2026-03-01: Fixed representative checkout success/receipt class-name binding by falling back to the selected class heading (`data-teqcidb-rep-selected-class-name`) when no accordion class item exists, eliminating 'Class Name: N/A' in the success details and generated PDF receipt.
321. 2026-03-01: Added representative checkout invalidation logic so changing Step 1/Step 2 selections after launching Accept Hosted clears the hosted form/token, resets iframe state, and prompts users to click Register & Pay Online again to generate an updated secure form with current totals and student mapping.
322. 2026-03-01: Fixed studenthistory enrollment-date persistence for successful payment registrations by correcting enrollmentdate insert formatting to string dates (`Y-m-d`) and now setting `adminapproved` to `Yes` on both self-registration and representative-created studenthistory rows.
...
326. 2026-03-02: Refactored class↔quiz association workflow so quiz tabs no longer assign classes; added a class-form 'Choose an Exam/Quiz' select on both create/edit flows, persisted selected quiz assignments by syncing the teqcidb_quiz_classes mapping table from class saves, and preloaded each class row with its assigned quiz in Edit & Manage Classes while keeping existing mapping-table storage intact.
327. 2026-03-02: Added a new Quizzes → Failed Quizzes admin tab with accordion rows for failed attempts (status = 1) that show student name/email/company and a per-question review table (student answer vs correct answer), plus a refresher-only Reset Quiz Attempt button placeholder for next-step wiring.
328. 2026-03-02: Enhanced Quizzes → Failed Quizzes by adding a collapsed-row Email Address column and wiring refresher-only Reset Quiz Attempt actions via AJAX to delete only matching quiz attempts/answer rows for the selected quiz + class + student scope, with inline spinner/status feedback and reload-on-success behavior.
329. 2026-03-02: Added a new Students → Student Forms tab that mirrors Edit Students search/pagination + accordion browsing in a read-only mode (no student edit actions), including dedicated search/list/pagination containers and JS data loading/rendering from teqcidb_read_student for forms-prep review.
330. 2026-03-02: Updated Students → Student Forms expanded rows to reuse the same Edit Students form layout/components and data population path, while forcing a read-only mode that disables all inputs/controls and hides save/delete actions so admins can view but not modify student records.
331. 2026-03-02: Added Student Forms expanded-panel certificate/wallet action UI above each read-only student form, with a two-row button layout (in-person actions with instructor-name inputs plus online/wallet actions) styled to match the provided admin mockup; functionality wiring is intentionally pending.
332. 2026-03-02: Tweaked Student Forms certificate action button styling to match requested visual defaults (initial color/border/background, initial line-height, top padding) and updated button-label sizing/spacing with fixed width/height and centered block alignment, scoped only to the teqcidb-student-forms-certificates controls.
333. 2026-03-02: Strengthened Student Forms certificate button CSS specificity under `#teqcidb-student-forms-list` (with targeted `!important` overrides) so requested min-height/flex layout and default color/border/background/line-height/padding styles consistently win over core `.wp-core-ui .button` rules.
334. 2026-03-02: Simplified Student Forms certificate CSS to the exact requested button and label declarations (including `!important` color/background/border/top-padding/white-space overrides and label max-width/line-height updates) for `.teqcidb-student-forms-certificates__button` and `__button-label`.
335. 2026-03-02: Updated Student Forms certificate item card styling to center-align item contents by adding `text-align: center;` to `.teqcidb-student-forms-certificates__item` for consistent button/label presentation.
336. 2026-03-02: Updated Student Forms certificate action buttons to include fixed sizing (`min-width: 220px; height: 95px;`) while retaining requested visual overrides for color/background/border/top padding/white-space.
337. 2026-03-02: Wired Students → Student Forms 'Generate Wallet Card' to the same jsPDF print workflow used on the dashboard Certificates tab by enqueueing jsPDF in admin, localizing wallet-card labels/logos, composing per-student wallet payloads from read-only student data, and opening a print-ready wallet-card PDF window from the expanded accordion action button.
338. 2026-03-02: Fixed Student Forms wallet-button click scope by binding `.teqcidb-student-forms-wallet-card-button` handlers on `#teqcidb-entity-list` (where wallet helpers are defined) and removing the out-of-scope document-level call that caused `handleStudentFormsWalletCardAction is not defined` in admin.js.
339. 2026-03-02: Added Student Forms backend initial in-person certificate generation by wiring the corresponding certificate button to jsPDF landscape output, reusing ADEM/Thompson logos, rendering certificate copy/layout from student data, and requiring the adjacent Instructor Name input value for the certificate's instructor field.
340. 2026-03-02: Normalized initial in-person certificate date rendering to `MM-DD-YYYY`, including support for both raw ISO (`YYYY-MM-DD`) and preformatted slash dates so class/training and expiration dates display consistently on generated certificates.
341. 2026-03-02: Refined Student Forms initial in-person certificate visuals by switching the certificate canvas background to pure white (`#ffffff`) and preserving the Thompson Engineering logo aspect ratio in the PDF render so the lower logo mark is no longer squashed.
342. 2026-03-02: Increased vertical spacing in the Student Forms initial in-person certificate PDF by moving the lower logo/detail block downward (ADEM + Initial Training Class/date/instructor lines) to create clearer separation from the 'for satisfactory completion of 8 instructional hours' text.
343. 2026-03-02: Added Student Forms backend refresher in-person certificate generation by reusing the initial in-person jsPDF layout/branding and wiring the refresher button to a dedicated render path with refresher-specific certificate copy (4 instructional hours, Refresher Training Class, Instructor Names) and `last_refresher_date` as the displayed class date.
344. 2026-03-02: Added Student Forms backend initial online certificate generation by wiring the initial-online button to a dedicated jsPDF landscape render that matches the online-initial certificate layout (top-corner logos, centered program/certificate headings, 'Online Initial Training' emphasis, QCI/expiration lines, and six-hour PDH footer copy).
345. 2026-03-02: Refined online-certificate outputs by raising the initial-online footer block away from the bottom border and added Student Forms backend refresher-online certificate generation with dedicated online-refresher copy/layout plus button wiring (including 4.0-hour PDH footer text).
346. 2026-03-05: Added activator support for a new `{$wpdb->prefix}teqcidb_quiz_slides` table (quiz/attachment ordering with active state and timestamps) and wired its CREATE TABLE statement into the existing activation dbDelta run list.
347. 2026-03-05: Added optional Create Quiz ZIP upload/import for refresher slides (multipart form field + translatable helper copy), then on quiz save now processes image ZIP entries in natural filename order into Media Library attachments and writes ordered active rows into `{$wpdb->prefix}teqcidb_quiz_slides`, with create-tab admin notices for imported slide counts and ZIP-import failure states.
348. 2026-03-05: Implemented refresher class-page slide-gate UI so quizzes with imported slides now ship slide runtime data (ordered image URLs + slide i18n labels), render slide meta/progress/image/Previous+Next controls before the quiz, and only reveal the quiz UI after the learner advances through the full slide set (initial client-side flow; persistence/completion handling remains for follow-up work).
349. 2026-03-05: Added a client-side 15-second slide pacing lock on refresher class pages so each Next Slide advance temporarily disables the next button before the learner can continue, while Previous Slide immediately clears the lock and re-enables forward navigation until the next advance.
350. 2026-03-05: Added a disabled-state hover tooltip on the refresher slide Next button so learners see 'Please study the slide and wait to proceed.' during the 15-second cooldown window.
351. 2026-03-05: Added a separate refresher-slide progress persistence lane with a dedicated teqcidb_slide_progress table, new REST slide-progress read/write endpoints, runtime slideProgress hydration, and class-page slide-boundary/visibility/unload saves that are isolated from quiz answer autosave state.
352. 2026-03-05: Updated refresher slide-gate progress-bar behavior so the visual bar now tracks the learner’s current slide position (current slide / total slides), including when navigating backward with Previous Slide.
353. 2026-03-05: Updated refresher class-page boot logic to skip slide-gate rendering and load the quiz immediately when persisted slide progress already indicates all slides were completed, avoiding an extra return-to-last-slide Start Quiz click on re-entry.
...
360. 2026-03-07: Closed a refresher-slide pacing bypass by tracking cooldown unlock timestamps per slide index, so back/forward navigation can return quickly across already-viewed slides while still enforcing the active cooldown on newly reached frontier slides until their original lock expires.
361. 2026-03-07: Updated Initial-class exam messaging/results UX by removing doubled percent symbols in score summaries, showing selected/correct answer labels instead of option keys, hiding missed-question review content on failed Initial exam submissions, adding a passed-result dashboard CTA in the in-page results card, and revising revisited Initial pass/fail class-page feedback copy (including removal of the QCI-number suffix on pass).
...
366. 2026-03-07: Updated class-page submitted-result rendering to hide missed-question review content for all failed outcomes (Initial and Refresher) on both immediate submit and return visits, while keeping review details visible for passed outcomes only.
367. 2026-03-07: Added Initial-exam pass side effects to assign missing QCI numbers (next sequential T#### parsed from existing qcinumber values, including entries with suffixes like " (QCP)"), roll student expiration_date forward two years (from existing date when present or from today when blank), set initial_training_date only when empty, and update matching studenthistory rows for the passed class to outcome=Passed, attended=Yes, adminapproved=Yes.
368. 2026-03-07: Extended quiz pass side effects to Refresher passes by reusing QCI-number/expiration-date updates, setting last_refresher_date to today, and marking the matching studenthistory row as Passed/Yes/Yes for the passed refresher class.
...
370. 2026-03-08: Updated Communications → Email Templates sample list by renaming "Welcome Aboard" to "New Registration & Payment", changing its trigger label to "Individual Registration/Payment", and removing the Consultation Follow Up, Membership Renewal Reminder, and Internal Alert: Payment Review accordion template rows.
371. 2026-03-08: Updated Communications email token behavior so token buttons insert into the currently active From Name, From Address, Subject, Body, or SMS field; renamed the Placeholder 1 token button to "Student First Name" with `{student_first_name}`; and switched preview/test token hydration to use the newest student record (with a first-name alias) for subject/body demonstrations.
372. 2026-03-08: Added additional Communications email tokens for Student Last Name, Student's Representative, Student Email, Student Company, Student Phone (Cell), and Student Phone (Office), including token alias mapping so previews/test sends resolve these values from the newest student record and representative data.
373. 2026-03-08: Removed generic "Placeholder N" token buttons from Communications → Email Templates so unnamed placeholder merge tags (including Placeholder 6 through Placeholder 28) no longer appear in the token list while keeping named student tokens available.
374. 2026-03-08: Updated Communications email tokens so Certification Expiration now maps to `{student_certification_expiration}` rendered from `expiration_date` as MM-DD-YYYY, and added a new Class Information token group (`{class_name}`, `{class_type}`, `{class_date}`, `{class_time}`, `{class_page}`, `{class_team_link}`) hydrated from the newest `teqcidb_classes` record for backend preview/test rendering.
375. 2026-03-08: Added a new Communications Email Templates “Formatting” token group with a Bold button that inserts `<strong></strong>` wrappers into the Email Body editor at the cursor/selection (auto-targeting body context), and updated token-button rendering/normalization to support wrap-style formatting actions while preserving bold rendering in Email Preview.
376. 2026-03-08: Refined Communications class-token formatting so `{class_type}` now outputs title-cased values (e.g., Initial/Refresher) and `{class_page}` now resolves to a full absolute URL on the current site domain when stored as a relative class path.
377. 2026-03-08: Updated Communications email body rendering so textarea line breaks are preserved in both Email Preview and test-send HTML output by converting newline characters to `<br>` while retaining allowed inline markup (e.g., `<strong>`).
378. 2026-03-08: Added a Formatting → Link action button in Communications Email Templates that requires highlighted Email Body text, prompts for a URL in a popup, and wraps the selection with an anchor tag for preview/send rendering; also localized new link-prompt/validation admin strings.
379. 2026-03-08: Added Communications Email Template CC/BCC inputs (supports multiple comma/semicolon/newline-separated addresses), wired Save Template + Send Test payloads to persist and reuse these fields, and added recipient-list sanitization plus `Cc`/`Bcc` headers for test sends.
380. 2026-03-08: Expanded Communications → Email Templates to 12 registration/payment-specific template accordions (student self-registration + representative/student and representative/self variants across Initial/Refresher and Virtual/In-Person), and updated accordion panel rendering so each `teqcidb-email-*` template now opens the full editor UI with tokens/preview/actions.
381. 2026-03-08: Updated admin Communications/Cron styling by removing the Email Templates title-cell `display:block`, increasing email-template accordion summary cell height/min-height to 80px, adding `tablenav-paging-text` horizontal margins, and adding consistent tablenav paging button/navspan spacing with `margin-left: 5px`.
382. 2026-03-08: Increased Communications Email Templates accordion summary row cell sizing from 80px to 110px (`min-height` + `height`) to provide additional vertical space for multi-line metadata.
383. 2026-03-08: Added Email Preview class-selection controls ("Preview with..." select populated with all classes newest-first + disabled-default option), wired class-token preview overrides on selection change, and added a context-aware "Export Class Attendees" CSV action (First Name/Last Name/Email Address) that enables only after class selection.
384. 2026-03-08: Refined Email Templates class-preview UX by normalizing class format labels to human-friendly "Online"/"In Person" in the Preview-with class selector and email-template trigger copy (UI only), and added preview link override logic so class page/team links update when switching the selected preview class.
385. 2026-03-08: Updated Email Templates preview class selector labeling so class types now display capitalized ("Initial"/"Refresher") in option text while preserving existing token values/behavior.
386. 2026-03-08: Added three new Email Templates class-cost merge tokens for registration/payment scenarios—{class_cost_total_transaction}, {class_cost_student_self}, and {class_cost_student_representative}—and wired preview class selection + latest class preview hydration to populate each token with formatted class cost values.
387. 2026-03-12: Applied the $50 association discount to logged-in student self-checkout in the student registration shortcode payment flow by defaulting single-student checkout to the current user for discount eligibility lookups when no multi-student payload is supplied.
388. 2026-03-12: Removed the TEQCIDB_MIN_EXECUTION_TIME delay system by deleting the plugin constant, removing the AJAX maybe_delay helper, and stripping all delay calls so AJAX handlers respond without enforced wait time.
389. 2026-03-12: Added automatic post-payment email delivery for qualifying self-registrations so successful student self-checkout for Initial Online/Virtual classes sends the "Student Self-Registration (Initial Online)" template to the logged-in student email with merged student/class transaction tokens.
...
403. 2026-03-13: Optimized Edit & Manage Classes performance by removing eager registered-student arrays from class list responses, adding a paginated admin AJAX endpoint for class registered students, and updating the class editor UI to load the first 40 students on demand with progressive “Load More Students” appends plus inline loading/error feedback.
404. 2026-03-14: Restricted the $50 association discount to Initial class registrations only by gating checkout discount calculations and per-student payment-history amount allocation on class type so Refresher registrations never receive the association discount.
405. 2026-03-14: Fixed registration email {class_page} token output to always resolve relative class URLs to full site URLs (matching Email Preview behavior) so sent emails include the active site domain instead of malformed host-only links.
406. 2026-03-14: Added automatic send support for the "Student Self-Registration (Refresher Online)" email template so single-student self-checkout now triggers that template only for successful Refresher + Online/Virtual class registrations, mirroring the existing Initial Online self-registration email behavior.
407. 2026-03-14: Added a new Communications Email Templates token section titled "Representative Information" with {representative_first_name} and {representative_last_name} tokens, and hydrated both tokens for preview/replacement using representative self-record first/last name values.
408. 2026-03-14: Added a new {individuals_registered} Communications token (under Representative Information) that renders email-safe <ul><li>Student Name (email)</li></ul> output using student data from the latest representative Register Students & Pay Online transaction for preview/demo hydration.
409. 2026-03-14: Added automatic sending for the "Representative Registration (Initial Online - Student)" template so each student in a representative multi-student Register Students & Pay Online transaction receives an individual email for Initial + Online/Virtual classes, with representative first/last name token hydration sourced from the paying representative.
410. 2026-03-14: Added automatic sending for the "Representative Registration (Initial Online - Representative)" template so qualifying Initial + Online/Virtual Register Students & Pay Online transactions now send a single confirmation email to the logged-in representative (not students), including transaction-specific {individuals_registered} token hydration from selected students.
411. 2026-03-14: Added automatic sending for the "Representative Registration (Refresher Online - Student)" template so each selected student in a representative Register Students & Pay Online transaction receives an individual email only for Refresher + Online/Virtual classes.
412. 2026-03-14: Added automatic sending for the "Representative Registration (Refresher Online - Representative)" template so qualifying Refresher + Online/Virtual Register Students & Pay Online transactions now send one confirmation email only to the logged-in representative, with transaction-specific {individuals_registered} token hydration.
413. 2026-03-14: Added automatic sending for the "Student Self-Registration (Initial In-Person)" template so successful self-checkout from the student registration shortcode now sends that template when class type is Initial and class format is in_person or inperson.
