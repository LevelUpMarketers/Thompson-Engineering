...
- Manage quiz records under **Quizzes**, switching between **Create a Quiz** and **Edit Quizzes** tabs. The create form now maps directly to the `teqcidb_quizzes` schema with fields for class linkage (`class_id`), quiz name, public token, status flag, and optional settings JSON metadata.
