...
if ( ! class_exists( 'TEQcidbPlugin_settings1_Form', false ) ) :
	...
	class TEQcidbPlugin_settings1_Form {
...

		{
			...
			$this->query_part_for_export = "SELECT * FROM $this->student_history_table WHERE attended = 'no' OR attended = 'null' AND amountpaid != ''";			



			$results = $wpdb->get_results( "SELECT * FROM $this->student_history_table WHERE attended = 'no' OR attended = 'null' AND amountpaid != '' LIMIT $this->pagination_place, $this->pagination_display_limit" );
			...
			foreach ($results as $key => $value) {
				...
				array_push( $studentidarray, $value->uniquestudentid );
			...
			foreach ($studentidarray as $key2 => $value2) {



				if( 0 === $key2 ){

					$query = $query . "SELECT * from " . $this->students_table . " WHERE uniquestudentid = '" . $value2 . "'";

				} else {

					$query = $query . " OR uniquestudentid = '" . $value2 . "'";
...
*/
			...
			$string1 = '<div class="teqcidb-display-search-ui-top-container">
							...
							<p class="teqcidb-tab-intro-para teqcidb-tab-intro-para-searchsort">Below is a list of all students who have paid some dollar amount for a class they did not attend.</p>

						</div>

						<button style="margin-top:30px;" data-query="' . $this->query_part_for_export . '" id="teqcidb-download-report-credits-list" class="teqcidb-search-ui-buttons">Download Report</button>';
