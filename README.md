...
- Class routes now render a full quiz runtime for logged-in students when a quiz is mapped to the class: all questions are displayed on a single page in quiz order, each question still uses the same answer-choice UI, submission requires all questions to be answered, and a lightweight hybrid autosave strategy now protects in-progress answers (debounced saves on answer changes, jittered periodic safety saves, and best-effort lifecycle saves on tab/page hide) while final submission scoring/persistence continues through `teqcidb_quiz_attempts` + `teqcidb_quiz_answer_items` (with legacy `teqcidb_quiz_answers` compatibility writes) (pass thresholds: Initial = 75%, Refresher = 80%).
...
- Class-page quiz clients support authenticated quiz progress saves and submission under `teqcidb/v1` via `POST /quiz/progress` and `POST /quiz/submit` (with legacy admin-ajax fallback actions `teqcidb_save_quiz_progress` and `teqcidb_submit_quiz_attempt`). Requests require `X-WP-Nonce` (`wp_rest`), login, class/quiz access checks, and attempt ownership checks. Save responses include `ok`, `attempt_id`, `saved_at`, and `message`; submit responses additionally include `score`, `passThreshold`, and `passed`; errors follow WordPress REST error schema (`code`, `message`, `data.status`).
