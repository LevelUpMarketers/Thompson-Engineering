...
243. 2026-02-11: Added Register & Pay Online buttons and unique embedded Accept Hosted iframes beneath each registration class details panel, plus a shared hidden token form and front-end JS bindings that request tokens and submit them into the selected iframe.
244. 2026-02-11: Added Accept Hosted token endpoint credential/env fallbacks and front-end payment startup diagnostics so Register & Pay Online now shows inline error feedback and logs concrete token request failures in the browser console.
245. 2026-02-11: Added backward-compatible Authorize.Net credential loading so payment gateway fields are read from nested or legacy flat teqcidb_api_settings option shapes, and clarified the missing-credentials token error to point to wp_options storage.
