...
- Manage quiz records under **Quizzes**, switching between **Create a Quiz** and **Edit Quizzes** tabs. The create form saves quiz records using a quiz name plus class-association checkboxes, while the edit tab now renders accordion rows for each saved quiz so admins can update quiz names and class checkbox associations. Selected class IDs are stored as a comma-separated value in `teqcidb_quizzes.class_id` (for example: `41,32,1,19`).
