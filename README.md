...
## Usage
 ...
 - Use the shortcode `[teqcidb_student_dashboard_shortcode]` to render the student dashboard login/create-account section. The create-account card explains that representatives must first create their own account before registering students and provides linked email/phone contact options for clarification. Logged-in students now also see a populated **Payment History** tab with class name, payment date/time, amount, transaction ID, invoice number, who each payment was applied to, and per-entry **Print Invoice**/**Download Invoice** actions. The **Class History** tab now links each class name and a class-page footer link in a new tab, and the **Your Students** tab now includes per-student print/download actions for Wallet Cards and Initial Certificates (shown when that student has a valid expiration date) plus Refresher Certificate actions only when that specific student has a passed Refresher history entry. Representative users also have a guided **Register Students & Pay Online** flow that requires selecting at least one assigned student plus one class before enabling checkout. In Step 2 of that representative flow, class options mirror front-end registration visibility by excluding classes marked hidden (`classhide = 1`), and checkout pricing is now calculated as `(class cost × selected students) - ($50 per selected student with any association)`. When a representative selects a Refresher class, selected students must have a non-empty QCI number and a non-expired certification date; ineligible selections now show descriptive Step 3 messaging and block checkout until resolved, and checkout expiration checks now evaluate end-of-day eligibility using the browser-supplied timezone offset (with site-timezone fallback). Step 3 now includes the same payment policy/instructions note shown in single-student registration before the checkout button. After representative checkout success, payment-history/student-history records are now written per selected student (while preserving the representative's aggregate transaction row with selected student WP IDs in `multiplestudents`). If Step 1 or Step 2 selections change after launching checkout, the hosted form is cleared and must be regenerated by clicking **Register & Pay Online** again so totals and student mappings stay in sync.
 - Use the shortcode `[teqcidb_student_registration_shortcode]` to show student class registration content and reuse the login/create-account experience for logged-out visitors. Registration class listings now auto-hide classes that are full (`registered >= classsize`) or already in the past (class start date earlier than today), and those classes are programmatically marked hidden (`classhide = 1`) during shortcode processing. For logged-in students without a QCI number (`teqcidb_students.qcinumber` is empty) or with an expired certification date (`teqcidb_students.expiration_date` before today, with same-day expiration still valid through 11:59 PM in the site timezone), the listing now also suppresses Refresher classes (`teqcidb_classes.classtype = refresher`).
- Use the shortcode `[teqcidb_qci_list_shortcode]` to render a paginated, searchable public-facing QCI list that includes only students with a non-empty QCI number and a populated `wpuserid` value, and shows accordion details for first/last name, company, city, state, QCI number, and expiration date.
 - Add the **Student** block in the block editor.
- Manage entities under **Students**, switching between **Create a Student**, **Edit Students**, **Newest Students**, and **Student Forms** tabs. Newest Students shows a read-only accordion list of the 20 most recently created records (no search/pagination), while Student Forms mirrors the Edit Students search/pagination accordion browsing experience in a read-only format so admins can review saved student data before generating forms outputs; each expanded Student Forms row includes certificate/wallet-card actions above the read-only form, with Generate Wallet Card now producing the same printable wallet-card PDF output used on the student dashboard's Certificates & Important Dates tab and Generate Completion Certificate (initial in-person) and Generate Completion Certificate (refresher in-person) now producing printable certificate PDFs that use the entered Instructor Name, and Generate Completion Certificate (initial online) plus Generate Completion Certificate (refresher online) now produce their own printable online certificate layouts.
- On **Students → Edit Students**, each editable student row now includes a **WordPress User Account** action between **Save Changes** and **Delete** that links to the matching `wp-admin/user-edit.php?user_id={wpuserid}` screen when `teqcidb_students.wpuserid` exists, or displays a disabled N/A action when it does not; the fourth accordion summary column now shows **QCI Number** from `teqcidb_students.qcinumber` instead of Cell Phone.
...
- Class-page quiz clients support authenticated progress saves and final submission under `teqcidb/v1` via `POST /quiz/save` and `POST /quiz/submit` (with legacy admin-ajax fallback actions `teqcidb_save_quiz_progress` and `teqcidb_submit_quiz_attempt`). Requests require `X-WP-Nonce` (`wp_rest`), login, class/quiz access checks, and attempt ownership checks. Save responses include `ok`, `attempt_id`, and `saved_at`; submit responses include `ok`, `attempt_id`, `score`, `passThreshold`, and `passed`; errors follow WordPress REST error schema (`code`, `message`, `data.status`).
...
- Class pages now auto-fade the resume notice after 10 seconds, hide the legacy welcome-description paragraph under the greeting panel, and render class resources from `teqcidb_classes.classresources` immediately below the Welcome panel (before the quiz section) as per-resource rows showing `Name (Type)` plus the resource URL link; empty classes display `No resources have been added for this class yet!`.
...
- Added a new **Student Initial Exam Passed** Communications email template that automatically sends to the logged-in student email after a successful Initial Exam pass result; delivery is queued via a short delayed single-event cron hook so grading/database updates complete first.
- Added a **Student Refresher Class Completed** editor to **Communications → Email Templates**; after a logged-in student completes all Refresher slides, the configured email is queued only after the pass outcome and refreshed certification dates have been persisted.
- Student Information token list now includes `{student_qci_number}` so templates can render each student's current QCI number directly in subject/body content.
