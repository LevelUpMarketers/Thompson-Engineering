...
Welcome to the Thompson Engineering QCI Database repository.
...
- Custom database table prefix is `teqcidb_`.
- Remove `includes/update-server/` before submitting to WP.org.
- The demo cron event `teqcidb_demo_cron_event` exists only to showcase the Cron Jobs tab—remove it for production or client builds.
- The Cron Jobs tab auto-detects cron hooks prefixed with `teqcidb_`; continue using this prefix for future scheduled tasks.
...
- Every admin tab should include the standardized title-and-description block (see `render_tab_intro()`), placed after `.teqcidb-top-message` and before tab-specific content.
